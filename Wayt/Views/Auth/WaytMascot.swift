...
struct WaytMascot: View {
    ...
    private var pinColor: Color { WaytTheme.mascotBodyTop }
    private var accentColor: Color { WaytTheme.mascotBodyBottom }
    ...
    private var pinBody: some View {
        ZStack {
            ...
            PinShape()
                ...
                .overlay(
                    PinShape()
                        .stroke(WaytTheme.mascotOutline, lineWidth: colorScheme == .dark ? 3.5 : 2.5)
                )
            ...
            Ellipse()
                .fill(WaytTheme.mascotFace)
                .overlay(
                    Ellipse()
                        .stroke(WaytTheme.mascotOutline, lineWidth: colorScheme == .dark ? 3.5 : 2.0)
                )
    ...
    private var antennaView: some View {
        VStack(spacing: 0) {
            ...
            Circle()
                ...
                .overlay(
                    Circle()
                        .stroke(WaytTheme.mascotOutline, lineWidth: 2.0)
                )
                .frame(width: size * 0.09, height: size * 0.09)

            // Antenna stick
            RoundedRectangle(cornerRadius: 1.5)
                .fill(WaytTheme.mascotOutline)
    ...
    private var leftEyeView: some View {
        ZStack {
            ...
            case .wink:
                ...
                WinkShape()
                    .stroke(WaytTheme.ink, style: StrokeStyle(lineWidth: 2.5, lineCap: .round))
...
#Preview("Mascot") {
    ScrollView {
        VStack(spacing: 20) {
            Text("Existing").font(.caption).foregroundStyle(WaytTheme.secondaryText)
            HStack(spacing: 40) {
                VStack {
                ...
                VStack {
                    WaytMascot(size: 90, expression: .excited)
                    Text("excited").font(.caption2)
                }
            }
            HStack(spacing: 40) {
                VStack {
                    WaytMascot(size: 90, expression: .happy)
                    Text("happy").font(.caption2)
                }
                VStack {
                    WaytMascot(size: 90, expression: .wink)
            ...
            Text("New — pick for \"Help your community\"").font(.caption).foregroundStyle(WaytTheme.secondaryText)
            HStack(spacing: 30) {
                VStack {
                    WaytMascot(size: 90, expression: .proud)
                    Text("proud").font(.caption2)
                }
                VStack {
                    WaytMascot(size: 90, expression: .cheerful)
                    Text("cheerful").font(.caption2)
                }
                VStack {
                    WaytMascot(size: 90, expression: .kind)
